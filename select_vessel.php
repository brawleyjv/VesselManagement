...
// Auto-select Test Vessel for demo users
if (isset($_SESSION['is_demo_mode']) && $_SESSION['is_demo_mode']) {
...
if ($_POST && isset($_POST['select_vessel'])) {
    ...
    // Verify vessel exists and is active - Fixed SQLite query
...
// Get available vessels (filtered for test admin) - Fixed SQLite query
if (is_test_admin()) {
    $vessels_sql = "SELECT VesselID, VesselName, VesselType FROM vessels WHERE IsActive = 1 AND VesselName = 'Test Vessel' ORDER BY VesselName";
} else {
    $vessels_sql = "SELECT VesselID, VesselName, VesselType FROM vessels WHERE IsActive = 1 ORDER BY VesselType, VesselName";
}

// Use proper SQLite query method
