...
// FIXED: Use your existing auth system instead of require_vessel_selection()
ensure_session();
require_login();

// FIXED: Get current user and vessel using your existing functions
$current_user = get_logged_in_user();
$active_vessel_id = get_active_vessel_id();
$active_vessel = get_active_vessel_info($conn);

if (!$active_vessel_id || !$active_vessel) {
    echo "<div class='error-message'>No vessel selected. Please select a vessel first.</div>";
    echo "<p><a href='index.php'>← Back to Dashboard</a></p>";
    exit;
}

$current_vessel = $active_vessel;  // For compatibility with existing code

// FIXED: Get available sides using simple query instead of get_vessel_sides()
$available_sides = [];
$equipment_type = $_GET['equipment'] ?? '';
if (!empty($equipment_type) && in_array($equipment_type, ['mainengines', 'generators', 'gears'])) {
    $sides_sql = "SELECT DISTINCT Side FROM $equipment_type WHERE VesselID = ? ORDER BY Side";
    $sides_stmt = $conn->prepare($sides_sql);
    $sides_stmt->bind_param('i', $active_vessel_id);
    $sides_stmt->execute();
    $sides_result = $sides_stmt->get_result();
    while ($side_row = $sides_result->fetch_assoc()) {
        $available_sides[] = $side_row['Side'];
    }
}
...
// FIXED: Get vessel-specific scales (simplified version)
$vessel_scales = [
    'rpm_min' => 0,
    'rpm_max' => 2000,
    'temp_min' => 0,
...
if (empty($equipment_type) || empty($side) || !in_array($equipment_type, ['mainengines', 'generators', 'gears'])) {
    ...
    // FIXED: Get data for graphing with proper VesselID filtering
    $sql = "SELECT * FROM $equipment_type WHERE VesselID = ? AND Side = ?";
    $params = [$active_vessel_id, $side];  // FIXED: Use active_vessel_id instead of current_vessel
    $types = 'is';
...
<body>
    ...
    <script>
        ...
        if (equipmentType === 'mainengines') {
            ...
            // SIMPLIFIED: Remove the AJAX call and just show gear data
            const dates = chartData.map(item => item.date);
            
            // Simplified gear chart without engine RPM correlation
            new Chart(document.getElementById('combinedChart'), {
                type: 'line',
                data: {
                    labels: dates,
